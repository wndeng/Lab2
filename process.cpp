...
Process::Process(int PID_, int AT_, int TC_, int CB_, int IO_, int prio): PID(PID_), AT(AT_), TC(TC_),  CB(CB_), IO(IO_), priority(prio-1), state(CREATED), FT(AT_), IT(0), CW(0), static_priority(prio), stateStartTime(AT), rem(TC_), ioFlag(false), currentCB(0) {};
...
void Process::changeState(State newState, int newTime) {
	...
	if(newState == READY && this->state == BLOCKED) {
	...
	if(this->state == RUNNING && newState == READY) {
		this->rem -= (newTime - this->stateStartTime);
		this->currentCB -= (newTime - this->stateStartTime);
		this->priority -= 1;
	}
